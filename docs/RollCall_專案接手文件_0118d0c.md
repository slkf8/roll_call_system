...
| Git worktree | clean |
| RC baseline | **RC10 tagged**（package + host-shell binary smoke 已完成；tag 已建立並單獨 push；main 尚未 push post-tag docs） |
| RC10 source baseline | `a3af39bb69ffc8137ec32f8edbfc6bdb248fa30d` |
| RC10 tag | **已建立並單獨 push**：lightweight tag `portable-release-candidate-10` → `a3af39bb69ffc8137ec32f8edbfc6bdb248fa30d`（local = remote，object type commit） |
...
- **Git 狀態**：RC11 source feature commits **已 push 至 `origin/main`**（main HEAD `38846dc67591494d3a00ee9cfc45f7da0a181b51`，含 docs acceptance commit）；RC11 portable package 與 named artifact **已建立**並通過 isolated packaged binary smoke；**RC11 lightweight tag `portable-release-candidate-11` 已建立並單獨 push** → source baseline `38846dc67591494d3a00ee9cfc45f7da0a181b51`（object type commit；local = remote；**非** pre-tag docs commit `d533329036c53bfa06f9b997a63f6cee80efaac5`）。
- **RC11 release artifact**：`release/RollCall_Portable_macOS_RC11.zip`（size `21222989`，SHA256 `5aceca9e173cd5a483c390d979fb840e5ea6e473ed9034942d1f120dda483300`，MD5 `b2409b2bd11538d6ee0e6a5c7c5a08cf`；generic / named BYTE-IDENTICAL；DB / lock exclusion PASS）。tag result evidence `phase_rc11_4_tag_result.txt`；詳見 [`RollCall_RC11_Release_BinarySmoke_Acceptance_2026-06-09.md`](RollCall_RC11_Release_BinarySmoke_Acceptance_2026-06-09.md)。
- **RC11 main push pending**：pre-tag docs commit `d533329036c53bfa06f9b997a63f6cee80efaac5` 尚未 push 至 `origin/main`；post-tag docs update 尚未 commit；main push 尚未執行。下一步為 post-tag docs commit → integrated main pre-push gate → 單次 `git push origin main`。
- **RC11 release out-of-scope**：Windows portable / 維修入口 / 實機 lock 驗收。
- **行為**：
...
- **RC10 package / host-shell packaged binary smoke 已完成 PASS（2026-06-08，pre-tag）**。
  ...
  - `portable-release-candidate-10` lightweight tag **已建立並單獨 push** → `a3af39bb69ffc8137ec32f8edbfc6bdb248fa30d`（local = remote，object type commit）。
  - pre-tag docs commit：`06d028af0b21427826eea259c103efa708a88540`；pre-tag docs commit **尚未 push 至 `origin/main`**；post-tag docs update **尚未 commit**；main **尚未 push post-tag docs**。
...
3. RC10 package / host-shell packaged binary smoke 已完成；lightweight tag `portable-release-candidate-10` 已建立並單獨 push → `a3af39bb69ffc8137ec32f8edbfc6bdb248fa30d`。下一步為 post-tag docs stage gate → docs-only local commit → integrated main pre-push gate → 單次 `git push origin main`。
