...
| RC11-4 tag | tag result（`phase_rc11_4_tag_result.txt`） |

摘要：source baseline `38846dc67591494d3a00ee9cfc45f7da0a181b51`；artifact `release/RollCall_Portable_macOS_RC11.zip`，SHA256 `5aceca9e173cd5a483c390d979fb840e5ea6e473ed9034942d1f120dda483300`（generic / named BYTE-IDENTICAL，DB / lock exclusion PASS）；isolated packaged binary smoke 狀態 `PASS_RC11_ISOLATED_PACKAGED_SMOKE_READY_FOR_RELEASE_DOCS`（GET-only API 全 HTTP 200、write API 未執行、精準 PID cleanup、`:8211` 已釋放、`:5173`/`:8000` 未操作）；formal-data policy `OPAQUE_PROTECTED_FORMAL_DATA_ZONE`、`formal_data_contents_inspected=no`。RC11 lightweight tag `portable-release-candidate-11` 已建立並單獨 push → `38846dc67591494d3a00ee9cfc45f7da0a181b51`（object type commit；local = remote；**非** pre-tag docs commit `d533329036c53bfa06f9b997a63f6cee80efaac5`），結果見 `phase_rc11_4_tag_result.txt`；pre-tag docs commit 尚未 push 至 `origin/main`，main push 尚未執行。evidence archive 未複製 synthetic runtime database、runtime lock 或 raw runtime log。詳見 [`RollCall_RC11_Release_BinarySmoke_Acceptance_2026-06-09.md`](RollCall_RC11_Release_BinarySmoke_Acceptance_2026-06-09.md)。
...
- RC10 release evidence 已封存於 `~/Documents/RollCall_AcceptanceArchives/RC10_BuildPreflight_2026-06-08_l6H5Ty/`：build result、binary metadata、package result、package inventory、named zip inventory、DB / lock exclusion scan、sandbox bind failure summary、host-shell smoke evidence、cleanup accepted variance、artifact hashes、`phase_rc10_5_tag_result.txt` 均已記錄。RC10 source baseline `a3af39bb69ffc8137ec32f8edbfc6bdb248fa30d`；artifact `release/RollCall_Portable_macOS_RC10.zip` SHA256 `e2c0fd84f6b3ddbb24c68dd932885e14894eefd400605d90fc3c015d008366c4`（MD5 `df79d74cd2a1bb2b25d621c2d3ae9288`、size 21222640）。Host-shell packaged binary smoke status：`PASS_RC10_PACKAGED_BINARY_SMOKE_WITH_EXACT_SIGINT_CLEANUP_READY_FOR_DOCS_GATE`；formal-data policy `OPAQUE_PROTECTED_FORMAL_DATA_ZONE`，formal-data contents inspected `no`。Cleanup variance：`TOOL_LIMITATION_TERMINAL_CTRL_C_KEYSTROKE` / `ACCEPTED_EXACT_SIGINT_TO_RECORDED_LISTENER_PID`。RC10 lightweight tag `portable-release-candidate-10` 已建立並單獨 push → `a3af39bb69ffc8137ec32f8edbfc6bdb248fa30d`（local = remote，object type commit）；main push 尚未執行。詳見 [`RollCall_RC10_Release_BinarySmoke_Acceptance_2026-06-08.md`](RollCall_RC10_Release_BinarySmoke_Acceptance_2026-06-08.md)。
